...
struct Actions {
    ...
    const Actions operator + (const Actions Jp) const{
	    Actions J2(Jr+Jp.Jr, Jz+Jp.Jz, Jphi+Jp.Jphi);
	    return J2;
    }
};
...
struct Frequencies {
    ...
    const Frequencies operator * (const double a) const{
	    Frequencies fr(a*Omegar, a*Omegaz, a*Omegaphi);
	    return fr;
    }
    ...
    const Frequencies operator + (const Frequencies fr) const{
	    Frequencies freqs2(Omegar+fr.Omegar, Omegaz+fr.Omegaz,
